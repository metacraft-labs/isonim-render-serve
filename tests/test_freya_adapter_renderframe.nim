## RS-M4 — direct unit test of the Freya adapter's `renderFrame`
## entry point against a real `FreyaRenderer` + a tiny tree built
## through the actual shim cdylib (no mocks).
##
## What this exercises:
##   * `FreyaRenderer.createElement` / `setAttribute` /
##     `appendChild` — the real shim FFI loaded via
##     `LD_LIBRARY_PATH`.
##   * `freya_adapter.newFreyaFrameSource`.
##   * `freya_adapter.renderFrame` — tree-derived synthetic
##     raster path (the RS-M4 capture approach, see the adapter's
...
suite "RS-M4: FreyaFrameSource.renderFrame (direct)":
  ...
  test "dimensions and payload length match the configured size":
    ...
    let fs = newFreyaFrameSource(r, root, width = 64, height = 48)
  ...
  test "non-empty tree produces at least one non-background pixel":
    ...
    let fs = newFreyaFrameSource(r, root, width = 80, height = 60)
    let frame = fs.renderFrame()
  ...
  test "empty tree (nil root) still yields valid frame":
    let r = FreyaRenderer()
    let fs = newFreyaFrameSource(r, nil, width = 16, height = 16)
    let frame = fs.renderFrame()
  ...
  test "mutating the tree changes the rendered pixels":
    ...
    r.appendChild(root, label)
    let fs = newFreyaFrameSource(r, root, width = 32, height = 32)
